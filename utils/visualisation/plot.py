...
def save_results(path_save, rgb_img, grasp_q_img, grasp_angle_img, depth_img=None, no_grasps=1, grasp_width_img=None):
    """
    Plot the output of a network
    :param rgb_img: RGB Image
    :param depth_img: Depth Image
    ...
    plt.title('RGB')
    plt.axis('off')
    plt.savefig(f'results_evaluate/{path_save}_rgb.png')
    ...
    if depth_img is not None:
        ...
        ax.set_title('Depth')
        ax.axis('off')
        fig.savefig('results_evaluate/depth.png')
    ...
    ax.imshow(rgb_img)
    for g in gs:
        g.plot(ax, color='y',linewidth=6)
    ax.set_title('Grasp')
    ax.axis('off')
    plt.savefig(f'results_evaluate/grasp.png')
    ...
    ax.axis('off')
    plt.colorbar(plot)
    plt.savefig(f'results_evaluate/quality.png')
    ...
    ax.axis('off')
    plt.colorbar(plot)
    plt.savefig(f'results_evaluate/angle.png')
    ...
    ax.axis('off')
    plt.colorbar(plot)
    plt.savefig(f'results_evaluate/width.png')
