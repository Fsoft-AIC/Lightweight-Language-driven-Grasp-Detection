...
if __name__ == '__main__':
    ...
    logging.info('Validation size: {}'.format(len(indices)))
    ...
    for network in args.network:
        ...
        with torch.no_grad():
            for idx, (x, y, didx, rot, zoom, text) in enumerate(test_data):
                xc = x.to(device)
                ...
                if args.iou_eval:
                    s = evaluation.calculate_iou_match(q_img, ang_img, 
                                                       test_data.dataset.get_gtbb(didx[0], rot[0], zoom[0]),
                ...
                if args.vis:
                    save_results(
                        path_save=network,
                        rgb_img=test_data.dataset.get_rgb(didx.item(), rot.item(), zoom.item(), normalise=False),
                        grasp_q_img=q_img,
